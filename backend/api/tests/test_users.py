...
class Test01UserAPI(TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.user_not_authenticated = User.objects.create(
            email='vpupkin3@yandex.ru',
            username='vasya3.pupkin',
            first_name='Вася3',
            last_name='Пупкин3',
            password='Qwerty123'
        )
        ...
        cls.client_authenticated.login(
            email='vpupkin2@yandex.ru', password='Qwerty123'
            )
        cls.token = Token.objects.create(
            user=cls.user_authenticated,
            key='0d33ffbceeb4d0465cc2dfc317e5bad3c0bd10ba'
        )
        cls.client_authenticated.credentials(
    ...
    def test_01_users_not_authenticated(self):
        response = Test01UserAPI.client.get('/api/users/')
        print('test01: ', response.status_code)
        self.assertNotEqual(
        ...
        self.assertEqual(
            response.status_code,
            200,
            ('Проверьте, что при GET запросе `/api/users/` без'
             ' токена авторизации возвращается статус 200')
        )
    
    def test_02_users_authenticated(self):
        response = Test01UserAPI.client_authenticated.get('/api/users/')
        print('test02: ', response.status_code)
        self.assertNotEqual(
            response.status_code,
            404,
            ('Страница `/api/users/` не найдена,'
        ...
        self.assertEqual(
            response.status_code,
            200,
            ('Проверьте, что при GET запросе `/api/users/`'
             ' c токеном авторизации возвращается статус 200')
        )
    
    def test_03_users_authenticated(self):
        response = Test01UserAPI.client_authenticated.get('/api/users/2/')
        print('test03: ', response.status_code)
        print(User.objects.all()[0].id)
        print(User.objects.all()[1].id)
        self.assertNotEqual(
            response.status_code,
            404,
            ('Страница `/api/users/` не найдена,'
    ...
    #     )
    #     users_count = User().objects.filter(username=admin.username).count()
